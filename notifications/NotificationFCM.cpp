	...
	const bool bFromNotification)
{
...

	{
		...
		root["message"]["topic"] = "dev";
		std::string szPostdata { root.toStyledString() };
		
		_log.Debug(DEBUG_EVENTSYSTEM, "FCM: Generated message: .%s.", szPostdata.c_str());
		...
		if (HTTPClient::POST(m_GAPI_FCM_PostURL, szPostdata, ExtraHeaders, sResult))
		{
			...
			if (ret)
			{
				...
				else
				{
					...
					_log.Debug(DEBUG_EVENTSYSTEM, "FCM: Message sent: .%s.", sResult.c_str());
					return true;
				}
			}
			else
